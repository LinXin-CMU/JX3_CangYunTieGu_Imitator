...
def Apply(player: Player, target: Target, dwSkillLevel):
    ...
    if buff_zy.lasting == 0 and cd_zhandao > 1*16 and cd_juedao > 1*16:
        player.CastSkill(13051, 1)
    ...
    if player.rage >= 65 and buff_dd.lasting > 0 and not player.IsHaveBuff(8424) and cd_zhandao == 0 and cd_duanma > 3*16:
        player.CastSkill(13050, 1)
    if buff_gy.lasting > 0 and player.rage >= 50:
        player.CastSkill(13050, 1)
...
cw_nuxue = skill_script(tSkillData, tSkillCoolDown, tSkillName, tDesc, nNeedGcdType, nNeedMinRage, nNeedPosState,
                          Apply)
